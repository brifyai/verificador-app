...
// Configuración de directorios - RUTA CORRECTA
const BASE_RECORDINGS_DIR = '/home/radioapp/radio-recorder/recordings';
...
async function getRadioInfo(radioId) {
    try {
        ...
        const response = await fetch(`${SUPABASE_URL}/rest/v1/radios?select=id,name,region,description,platform,status,stream_url&vps_id=eq.${encodeURIComponent(radioId)}`, {
            method: 'GET',
            headers: {
        ...
        const response = await fetch(`${SUPABASE_URL}/rest/v1/recordings`, {
            ...
            body: JSON.stringify({
                ...
                metadata: {
                    ...
                    radio_stream_url: recordingData.radio_stream_url,
...
app.get('/api/recordings', async (req, res) => {
    try {
        ...
        if (!await fs.pathExists(BASE_RECORDINGS_DIR)) {
            log(`⚠️ Directorio no existe: ${BASE_RECORDINGS_DIR}`);
            return res.json({
                status: 'success',
                recordings: [],
                count: 0,
                message: 'Directorio de grabaciones no existe'
            });
        }
        
        // Leer todas las carpetas de fecha
        const dateFolders = await fs.readdir(BASE_RECORDINGS_DIR);
        log(`📅 Carpetas de fecha encontradas: ${dateFolders.length}`);
...
app.post('/api/start-recording', async (req, res) => {
    try {
        ...
        log(`🎬 Ejecutando FFmpeg: ffmpeg ${ffmpegArgs.join(' ')}`);
        ...
        ffmpegProcess.on('close', async (code) => {
            if (code === 0) {
                ...
                try {
                    ...
                    log(`📦 Archivo creado: ${outputPath} (${stats.size} bytes)`);
                    ...
                    if (radioInfo) {
                        ...
                        const recordingData = {
                            ...
                            radio_stream_url: radioInfo.stream_url || stream_url
                        };
...
app.get('/recordings/:date/:radioId/:filename', async (req, res) => {
    try {
        ...
        log(`📥 Solicitando archivo: ${filePath}`);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    log(`📁 Directorio base: ${BASE_RECORDINGS_DIR}`);
    log(`📊 Estructura: /recordings/YYYY-MM-DD/radio_id/`);
});
