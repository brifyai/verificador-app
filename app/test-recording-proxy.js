// Script para probar la solución definitiva del proxy de grabación
require('dotenv').config({ path: '.env' });

async function testRecordingProxy() {
  console.log('🧪 Probando solución definitiva del proxy de grabación...\n');
  ...
  try {
    ...
    const startResponse = await fetch(`${baseUrl}/api/recording-proxy`, {
      method: 'POST',
      headers: {
    ...
    if (startData.success) {
      ...
      const statusResponse = await fetch(`${baseUrl}/api/recording-proxy`);
      ...
      if (statusData.count > 0) {
        ...
        const stopResponse = await fetch(`${baseUrl}/api/recording-proxy?radio_id=${testRadio.radio_id}`, {
          method: 'DELETE'
        });
        ...
        if (stopData.success) {
          ...
          const finalStatusResponse = await fetch(`${baseUrl}/api/recording-proxy`);
    ...
    for (const testCase of testCases) {
      ...
      try {
        const response = await fetch(`${baseUrl}/api/recording-proxy`, {
          method: 'POST',
...
// Ejecutar prueba
testRecordingProxy().catch(error => {
