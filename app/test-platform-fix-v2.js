 ...
 * Este script prueba que los cambios de plataforma se guarden correctamente
 */
...
async function updateRadioPlatform(radioId, newPlatform) {
  ...
  try {
    // Primero obtener la radio actual
    const getResponse = await makeRequest(`${BASE_URL}/api/radios-direct/${radioId}`);
    
    if (getResponse.status !== 200) {
      console.log('❌ Error obteniendo radio para actualizar:', getResponse.data);
      return false;
    }

    const currentRadio = getResponse.data;
    
    // Actualizar solo la plataforma
    const updateData = {
      ...currentRadio,
      platform: newPlatform
    };

    // Usar el endpoint PUT correcto
