// Auditoría simplificada usando Supabase Direct
...
async function auditarTablas() {
  console.log('🔍 AUDITORÍA DE SUPABASE DIRECT');
  console.log('====================================');
  ...
  try {
    // Obtener todas las tablas consultando información del schema
    console.log('📋 Obteniendo tablas de Supabase...');
    
    // Query para obtener todas las tablas del schema public
    const { data: tablas, error } = await supabaseDirect.client
      .from('information_schema.tables')
      .select('table_name')
      .eq('table_schema', 'public')
      .neq('table_name', 'pg_stat_statements')
      .neq('table_name', 'pgmigrations');
    ...
    for (const tabla of tablas) {
      ...
      // Solo auditar tablas que esperamos
      if (!SCHEMA_ESPERADO[tableName]) {
        console.log(`⚠️  Tabla no esperada: ${tableName}`);
        continue;
      }
      ...
      const { data: columnas, error: columnError } = await supabaseDirect.client
      ...
      if (columnError) {
        console.error(`❌ Error obteniendo columnas de ${tableName}:`, columnError.message);
        continue;
      }
      ...
      for (const columna of columnas) {
        ...
        if (tipoEsperado) {
          ...
          if (tipoEsperado === 'text' && (tipoNormalizado.includes('text') || tipoNormalizado.includes('varchar') || tipoNormalizado.includes('character'))) tipoCoincide = true;
          ...
          else if (tipoEsperado === 'real' && (tipoNormalizado.includes('real') || tipoNormalizado.includes('float') || tipoNormalizado.includes('double'))) tipoCoincide = true;
          else if (tipoEsperado === 'bigint' && tipoNormalizado.includes('bigint')) tipoCoincide = true;
          else if (tipoEsperado === 'timestamp' && (tipoNormalizado.includes('timestamp') || tipoNormalizado.includes('datetime'))) tipoCoincide = true;
          else if (tipoEsperado === 'jsonb' && (tipoNormalizado.includes('jsonb') || tipoNormalizado.includes('json'))) tipoCoincide = true;
...
if (require.main === module) {
  auditarTablas().catch(console.error);
}

module.exports = { auditarTablas };