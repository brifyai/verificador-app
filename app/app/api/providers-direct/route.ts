...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verificar si existe
    const existing = await supabaseDirect.request(`api_configurations?select=id&provider=eq.${providerCode}`);
    ...
    const configData = {
      ...
      metadata,
      created_at: existing.length > 0 ? undefined : new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
    ...
    if (existing.length > 0) {
      ...
      result = await supabaseDirect.request(
        `api_configurations?id=eq.${existing[0].id}`,
        {
          method: 'PATCH',
      ...
      // Crear
      result = await supabaseDirect.request('api_configurations', {
        method: 'POST',
        body: JSON.stringify(configData),
        headers: { 'Prefer': 'return=representation' }
      });
    ...
    return NextResponse.json({ provider: toApiProvider(result[0]) });
  } catch (error: any) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    // Buscar existente por id o provider
    let existing = null;
    if (id) {
      const existingList = await supabaseDirect.request(`api_configurations?select=*&id=eq.${id}`);
    ...
    const prev = existing || (await supabaseDirect.request(`api_configurations?select=*&provider=eq.${providerCode}`))[0];
    ...
    const updated = await supabaseDirect.request(
      id ? `api_configurations?id=eq.${id}` : `api_configurations?provider=eq.${providerCode}`,
      {
        method: 'PATCH',
        body: JSON.stringify(configData),
        headers: { 'Prefer': 'return=representation' }
    ...
    return NextResponse.json({ provider: toApiProvider(updated[0]) });
  } catch (error: any) {
