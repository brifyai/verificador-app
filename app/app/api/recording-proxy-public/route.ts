...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`🎙️ [RECORDING-PROXY-PUBLIC] Iniciando grabación para radio ID: ${actualId}`);
    ...
    try {
      ...
      console.log(`⚠️ [RECORDING-PROXY-PUBLIC] VPS no disponible: ${(vpsError as Error).message}`);
    }

    // Si el VPS falla, simular grabación localmente
    console.log(`🔄 [RECORDING-PROXY-PUBLIC] Simulando grabación local para radio ${actualId}`);
    ...
    console.error('❌ [RECORDING-PROXY-PUBLIC] Error:', error);
    return NextResponse.json(
      { 
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(`🛑 [RECORDING-PROXY-PUBLIC] Deteniendo grabación para radio ID: ${actualId}`);
    ...
    try {
      ...
      if (vpsResponse.ok) {
        console.log(`✅ [RECORDING-PROXY-PUBLIC] VPS detuvo la grabación correctamente`);
      }
    } catch (vpsError) {
      console.log(`⚠️ [RECORDING-PROXY-PUBLIC] Error al detener en VPS: ${(vpsError as Error).message}`);
    }
    ...
    console.error('❌ [RECORDING-PROXY-PUBLIC] Error al detener:', error);
    return NextResponse.json(
      { 
        error: 'Error interno del servidor',
        message: (error as Error).message 
...
export async function GET() {
  try {
    console.log(`📊 [RECORDING-PROXY-PUBLIC] Consultando estado de grabaciones`);
    ...
    try {
      ...
      console.log(`⚠️ [RECORDING-PROXY-PUBLIC] Error obteniendo estado VPS: ${(vpsError as Error).message}`);
    }
    ...
    recordingState.forEach((recording, id) => {
      if (recording.isRecording) {
        localRecordings[id] = {
          ...
          start_time: recording.startTime?.toISOString(), // Cambiado a start_time para compatibilidad
          status: 'recording',
          recording_id: id.toString(),
          started_at: recording.startTime?.toISOString(), // Mantener también started_at
          mode: 'local_simulation'
        };
      }
    });
    ...
    console.error('❌ [RECORDING-PROXY-PUBLIC] Error consultando estado:', error);
    return NextResponse.json(
      { 
        error: 'Error interno del servidor',
        message: (error as Error).message 
