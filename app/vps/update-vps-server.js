// Script para actualizar el servidor VPS con URLs válidas
const fs = require('fs');

// URLs de radio chilenas actualizadas y verificadas
const VALID_RADIO_URLS = {
  'radio1': {
...
// Generar código para actualizar el endpoint del frontend
function generateUpdatedEndpoint() {
  return `
// Función para obtener datos completos de radio con URLs válidas
function getRadioData(radioId) {
  const validUrls = ${JSON.stringify(VALID_RADIO_URLS, null, 2)};
...
// Crear script de actualización para la VPS
function generateVPSUpdateScript() {
  return `#!/bin/bash
...
// Generar archivos
console.log('📦 Generando archivos de actualización...');

// Script de actualización VPS
const vpsScript = generateVPSUpdateScript();
fs.writeFileSync('./update-vps.sh', vpsScript);
console.log('✅ update-vps.sh creado');

// Código para el frontend
const frontendCode = generateUpdatedEndpoint();
fs.writeFileSync('./frontend-radio-urls.js', frontendCode);
console.log('✅ frontend-radio-urls.js creado');

console.log('\n🎯 INSTRUCCIONES:');
console.log('1. En la VPS, ejecuta: chmod +x update-vps.sh && ./update-vps.sh');
console.log('2. Actualiza el endpoint del frontend con las URLs válidas');
console.log('3. Prueba el botón de monitoreo desde el dashboard');

module.exports = { VALID_RADIO_URLS, generateUpdatedEndpoint };
